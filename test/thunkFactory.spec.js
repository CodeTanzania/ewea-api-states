...
import createThunkFor, { createReportThunkFor } from '../src/factories/thunk';
...
describe('Thunk Factory', () => {
  describe('#createThunkFor', () => {
    ...
    it('should dispatch actions when loadMore resources succeed', () => {
      const store = mockStore({
        focalPeople: {
          list: [],
          hasMore: true,
    ...
    it('should not dispatch any action when hasMore is false', () => {
      ...
      expect(
        store.dispatch(
          focalPersonThunks.loadMoreFocalPeople(onSuccess, onError)
        )
      ).toBeUndefined();
    });

    it('should dispatch actions when refresh resources succeed', () => {
      const store = mockStore({
        focalPeople: {
          list: [],
          filter: { name: 'Test' },
    ...
    it('should reload resources when clearing part of filters succeed', () => {
      ...
      return store
        .dispatch(
          focalPersonThunks.clearFocalPersonFilters(onSuccess, onError, [
            'name',
          ])
        )
        .then(() => {
    ...
    it('should dispatch required actions when search resources succeed', () => {
      ...
      return store
        .dispatch(
          focalPersonThunks.searchFocalPeople('Test', onSuccess, onError)
        )
        .then(() => {
          expect(store.getActions()).toEqual(expectedActions);
          expect(onSuccess).toHaveBeenCalledTimes(1);
    ...
    it('should dispatch actions when paginate resource succeed', () => {
      const store = mockStore({
        focalPeople: {
          list: [],
          filter: {},
  ...
  describe('#createReportThunkFor', () => {
    it('should dispatch required actions when get report succeed', () => {
      const store = mockStore({
        overviewsReport: {
          overviews: {},
        },
      ...
      getOverviewsReport.mockResolvedValueOnce(mockData);
      const onSuccess = jest.fn();
      const onError = jest.fn();

      const overviewsThunks = createReportThunkFor('overview');
      const expectedActions = [
        {
          type: 'overviewReport/getOverviewsReportRequest',
          payload: undefined,
        },
        { type: 'overviewReport/getOverviewsReportSuccess', payload: mockData },
      ];

      return store
        .dispatch(overviewsThunks.getOverviewsReport({}, onSuccess, onError))
        .then(() => {
          expect(store.getActions()).toEqual(expectedActions);
          expect(onSuccess).toHaveBeenCalledTimes(1);
          expect(onError).toHaveBeenCalledTimes(0);
    ...
    it('should dispatch required actions when get resources fails', () => {
      ...
      getOverviewsReport.mockRejectedValueOnce(error);
      const onSuccess = jest.fn();
      const onError = jest.fn();

      const overviewReportThunks = createReportThunkFor('overview');
      const expectedActions = [
        {
          type: 'overviewReport/getOverviewsReportRequest',
          payload: undefined,
        },
        { type: 'overviewReport/getOverviewsReportFailure', payload: error },
      ];

      return store
        .dispatch(
          overviewReportThunks.getOverviewsReport({}, onSuccess, onError)
        )
        .then(() => {
          expect(store.getActions()).toEqual(expectedActions);
          expect(onSuccess).toHaveBeenCalledTimes(0);
